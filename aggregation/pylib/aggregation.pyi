from typing import TypeAlias, Tuple, Any, Dict, Mapping

# UserInput: TypeAlias = Round0 | Round1 | Round2 | Round3 | Round4
Messages: TypeAlias = Mapping[int, Any] # UserInput
Vector: TypeAlias = list[int]
SignPublicKey: TypeAlias = bytes
SignSecretKey: TypeAlias = bytes
ServerOutputSerialized: TypeAlias = Messages | Vector

class PublicKeysWrapper:
    def insert(self, u: int, pk: SignPublicKey) -> None: ...

class UserWrapper:
    def __new__(cls, id: int, threshold: int, sign_pk: SignPublicKey, sign_sk: SignSecretKey, vec: list[int], others_sign_pks: PublicKeysWrapper) -> None: ...
    def serialize_state(self) -> str: ...
    def recover_state(self, state: str) -> None: ...
    def round(self, msg: Any, input: list[int]) -> None: ...

class ServerOutputWrapper:
    def __new__(cls, wrapped: ServerOutputSerialized) -> None: ...
    def is_messages(self) -> bool: ...
    def is_vector(self) -> bool: ...
    def get_messages(self, arg: Any) -> Dict[int, bytes]: ...
    def get_vector(self) -> list[int]: ...

class ServerWrapper:
    def __new__(cls, threshold: int, vec_len: int) -> None: ...
    def serialize_state(self) -> str: ...
    def recover_state(self, state: str) -> None: ...
    def recv(self, id: int, input: list[int]) -> None: ...
    def round(self) -> ServerOutputWrapper: ...

def round0_msg(msg: Any) -> bytes: ...
def gen_keypair() -> Tuple[SignPublicKey, SignSecretKey]:
    """
    Generate a keypair (public key, secret key).
    """
